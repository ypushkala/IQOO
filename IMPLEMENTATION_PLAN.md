...
## Bug fix + settings/onboarding redesign (2026-09-22)

**Bug found and fixed:** "Help a family member set up" crashed with `NoSuchMethodError: text(FZ)` — a stale incremental-build artifact. `HelperActivity.kt` was never recompiled after `UiTheme.text()` gained a third parameter earlier in the session (a known class of Kotlin/Gradle incremental-compiler issue, consistent with the "daemon terminated unexpectedly" hiccups seen earlier). Fixed with `./gradlew clean` + rebuild; confirmed no longer crashes.

**Redesign, following the brief exactly** (trustworthy Android utility app, Pixel/Material settings influence, no giant buttons standing in for every control type):
- **New component system** (`Rows.kt`): `switchRow` (real `android.widget.Switch`, takes effect immediately), `navRow` (title + optional description + chevron, opens elsewhere), `statusRow` (a fact, never tappable), `infoRow` (title + description, no control at all), `sectionHeader`. Plain rows on the page background with a thin divider between them — no card wrapping, matching stock Android Settings rather than the app's own card style (health card, summary card), which stayed as cards since those are a different UI concept (status panels, not a settings list).
- **`SettingsActivity` rebuilt** into the five sections asked for — Protection (analyse-calls switch, caller-notification switch, blocked-numbers count, scam-list update), Accessibility (language, accessibility-mode switch, family alerts, language files), Help & testing (call history, practice, report missed, help a family member), Privacy (three short facts + a live "Internet access: verified NOT requested" status row, reusing the same `TrustCheck` built for the onboarding trust step + a "Manage CallGuard data" row), Danger zone (turn off protection, now behind a confirmation, visually separated).
- **Destructive styling**: new `UiTheme.dangerButton()` — outlined in the danger colour, never a filled block; used only for "Turn off protection".
- **Sentence case app-wide, for free**: found that the ALL CAPS everywhere came from the platform `Button` style's default `textAllCaps`, not from the strings themselves (which were already sentence case). Setting `isAllCaps = false` in `UiTheme.button()`/`primary()`/new `dangerButton()` fixed every button in the app in one place — no string rewrites needed except where the spec asked for genuinely different wording (a few Settings-list row labels, e.g. "Call history"/"Practice a call"/"Report a missed detection").
- **Onboarding (Get ready)**: reordered the visual hierarchy — a small "Get ready" eyebrow, "Step X of Y", a thin proportional progress bar (two weighted plain `View`s, no new dependency), then the step's own heading as the true large anchor of the screen. "Read this aloud" now uses a real icon (`ic_volume`) instead of an emoji.
- **`HelperActivity` rebuilt**: numbered steps (quiet blue numeral, not another bold heading) instead of three stacked giant all-caps blocks; the two share actions are bordered (not filled) since a screen with two "primary-looking" filled buttons competes with itself.
- **New icons** (`ic_home`, `ic_history`, `ic_settings`, `ic_chevron_right`, `ic_volume`, `ic_check_circle`): hand-authored VectorDrawables, same restrained stroke style as the earlier icon set — no bitmaps, no new dependency.
- **Status bar overlap fixed** on every screen touched in this pass (`UiTheme.avoidStatusBar()`, a `WindowInsetsCompat` listener adding the status bar's height as extra top padding) — a pre-existing issue across the whole app, fixed here on Home, Settings, Get Ready and the Helper screen since those were the ones being verified.

**Verified on-device (Galaxy F17, real hardware, no emulator available):**
- Default width (~384dp): Home, Settings (full scroll, every section), the danger-zone confirmation dialog, and Get Ready's progress bar at two different steps — all confirmed.
- **360dp and 412dp**, simulated on the same physical device via `adb shell wm density` (a standard technique for testing responsive layouts without an emulator; density restored to the original 450 afterward): Home, Settings (full scroll) and History confirmed clean at both widths — text wraps naturally, nothing clipped, the bottom nav never overlaps content.
- `HelperActivity` (the reported bug) confirmed fixed and redesigned at 412dp.
- Not set up: a real Android emulator/AVD (none installed, no system image, and downloading one was judged not worth the time for this pass) — the density-override technique was used instead and covers the same layout question.
